...
class Sam3Image(torch.nn.Module):
    ...
    def forward_grounding(
        ...
        exemplar_feats: torch.Tensor = None,  # [추가] 외부 이미지 임베딩 입력
    ):
        with torch.profiler.record_function("SAM3Image._encode_prompt"):
            # 기존 프롬프트(텍스트, 포인트 등) 인코딩
            # prompt Shape: [Seq_Len, Batch, Hidden_Dim] (예상)
            # prompt_mask Shape: [Batch, Seq_Len]
            prompt, prompt_mask, backbone_out = self._encode_prompt(
                backbone_out, find_input, geometric_prompt
            )

        # ============================================================
        # [추가] Exemplar Feature 주입 (Injection)
        # ============================================================
        if exemplar_feats is not None:
            # exemplar_feats: [S_ref, B, C]
            
            # 1. 배치 사이즈 확인 (혹시 모를 불일치 방지)
            if prompt.shape[1] != exemplar_feats.shape[1]:
                # 보통 Processor에서 잘 맞추겠지만, 다를 경우 여기서 처리하거나 에러
                pass 

            # 2. 마스크 생성
            # PyTorch Convention: 0(False) = Valid, 1(True) = Padding
            # Exemplar는 유효한 정보이므로 0으로 채운 마스크를 만듭니다.
            S_ref, B, C = exemplar_feats.shape
            ref_mask = torch.zeros((B, S_ref), dtype=prompt_mask.dtype, device=prompt_mask.device)
            
            # 3. 이어 붙이기 (Concatenate)
            # Prompt Sequence 뒤에 Exemplar Sequence를 붙입니다.
            # prompt: [S, B, C] + [S_ref, B, C] -> [S+S_ref, B, C]
            prompt = torch.cat([prompt, exemplar_feats], dim=0)
            
            # Mask: [B, S] + [B, S_ref] -> [B, S+S_ref]
            prompt_mask = torch.cat([prompt_mask, ref_mask], dim=1)
            
            # (디버깅 확인용 - 필요시 주석 해제)
            # print(f"🔗 Merged Prompts: Text/Geo({prompt.shape[0]-S_ref}) + Exemplar({S_ref})")

        # ============================================================

        # Run the encoder (이제 Exemplar가 포함된 prompt가 들어갑니다)
