...
def agent_inference(
    ...
    if temp_img.mode != "RGB":
        img_path = img_path.rsplit('.', 1)[0] + "_converted.jpg"
        temp_img.convert("RGB").save(img_path)
        print(f"⚠️ Image converted to RGB: {img_path}")
        ...
        if tool_call["name"] == "segment_phrase":
            ...
            if current_text_prompt in USED_TEXT_PROMPTS:
                ...
                if not os.path.exists(PATH_TO_LATEST_OUTPUT_JSON):
                    raise FileNotFoundError(f"❌ SAM service failed to create output JSON: {PATH_TO_LATEST_OUTPUT_JSON}")
