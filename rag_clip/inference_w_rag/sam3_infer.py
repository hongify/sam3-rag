...
# sam3_infer.py (아래 시각화 부분 수정)

# ... (앞부분 sam3_inference 등의 코드는 동일) ...
...
def run_comprehensive_experiment(processor, image_path, text_prompt, reference_infos, output_folder_path="sam3_output"):
    ...
    # 💡 Row 0, 1, 2: References (최대 12개 유동적 처리)
    for i in range(12):
        row = i // 4
        col = i % 4
        ax = axes[row, col]
        ...
        if reference_infos and i < len(reference_infos):
            ...
            # 남는 칸은 깔끔하게 빈칸으로 둠
            if i == 0 and (not reference_infos or len(reference_infos) == 0): 
                ax.text(0.5, 0.5, "No References", ha='center', va='center', fontsize=20)
            ax.set_title("") 

    # 💡 Row 3: Results (4가지 시나리오 결과)
